...
In the **Terminal**, `smoke-signal.py` reads this straight from the transcript every turn, so it is always accurate. In the **app**, the footer is typed by Claude, who used to ESTIMATE the percentage and drift badly (once showing 62% at a true 82%). That is now fixed by `ctx-hook.py`, a UserPromptSubmit hook that mirrors the effort hook. It runs the same transcript math (input + cache creation + cache read = the context as of the user's send), but it then PROJECTS one step forward: a footer is text Claude types, so its own reply is not yet on the transcript when the number is written, and the live meter the user pulls up after the reply is always higher by that reply. To close that gap, the hook measures the user's own recent per-turn growth from the transcript (the average of the last few real deltas, ignoring compaction drops) and adds it, so the footer lands on what the meter reads AFTER the reply. On a normal conversational turn it matches; on a turn with unusually large tool output (big file reads, many edits) the growth cannot be known in advance, so it can under or over shoot that one turn. It injects the exact projected ctx string for Claude to print, writes `~/.claude/active-ctx` (`show|window|pct|measured`) as a fallback, and tells Claude when usage is past 75% so the red diff block and fire widget fire on the true threshold. Like all hooks, it loads at session start, so a fresh install or hook edit needs a Claude Code restart.
